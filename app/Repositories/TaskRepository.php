...
class TaskRepository extends BaseRepository
{
    ...
    public function store($input)
    {
        ...
        $input['estimate_time'] =
        !empty($input['estimate_time_hours']) ? $input['estimate_time_hours'] :
        (!empty($input['estimate_time_days']) ? $input['estimate_time_days'] :
        $input['estimate_time_minutes']);
...
//                array_push($input['assignees'], getLoggedInUserId());
                    ...
                    if ($user != getLoggedInUserId()) {
                        UserNotification::create([
                            ...
                            'description' => $task->title.' assigned to you',
                             'link' => url('/projects/' . $task->project_id ), 
                            'type' => Task::class,
                            'user_id' => $user,
                        ]);
                    }
    ...
    public function update($input, $id)
    {
        ...
        // Check if status is being changed and validate required fields
        if (isset($input['status'])) {
            // Get the status record to determine status type
            $statusRecord = \App\Models\Status::where('status', $input['status'])->first();
            ...
            if ($isCompletedStatus) {
                if ($task->due_date == null) {
                    throw new UnprocessableEntityHttpException('Task must have a due date to be marked as completed.');
                }
                if ($task->estimate_time == null) {
                    throw new UnprocessableEntityHttpException('Task must have an estimated time to be marked as completed.');
                }
                if ($task->taskAssignee->isEmpty()) {
                    throw new UnprocessableEntityHttpException('Task must be assigned to at least one employee to be marked as completed.');
                }
            }
            
            // If moving to in-progress status, validate required fields
            if ($isInProgressStatus) {
                if ($task->due_date == null) {
                    throw new UnprocessableEntityHttpException('Task must have a due date to be marked as in-progress.');
                }
                if ($task->estimate_time == null) {
                    throw new UnprocessableEntityHttpException('Task must have an estimated time to be marked as in-progress.');
                }
            }
            
            // Set completed_on based on status
            $input['completed_on'] = $isCompletedStatus ? Carbon::now() : null;
        }
        
        $input['description'] = is_null($input['description']) ? '' : $input['description'];
        if (array_key_exists('estimate_time_type', $input)) {
            $input['estimate_time'] =
            !is_null($input['estimate_time_hours']) ? $input['estimate_time_hours'] :
            (!is_null($input['estimate_time_days']) ? $input['estimate_time_days'] :
            ($input['estimate_time_minutes']));
            $input['estimate_time_type'] = is_null($input['estimate_time']) ? null : $input['estimate_time_type'];
        }
...
//                array_push($user['task_assignees'], getLoggedInUserId());
    ...
    {   // rayhan modified start
        ...
        // Support composite keys like "{tagId}_{deptId}" as existing tags.
        // Extract the numeric tagId part and treat it as an existing tag.
        $existingTagIds = collect($tags)
            ->filter(function ($value) {
                // numeric id or composite like 142_0
                return is_numeric($value) || (is_string($value) && preg_match('/^\d+(_\d+)?$/', $value));
            })
            ->map(function ($value) {
                if (is_numeric($value)) {
                    return (int)$value;
                }
                // composite id: take the first segment before underscore
        ...
        // New tags are any that are not numeric/composite (i.e., free-typed strings via select2 tags:true)
        ...
        // First sync existing tag IDs
        $task->tags()->sync($existingTagIds->all());

        // Then create and attach any new tag names
        if ($newTags->count() > 0) {
            $createdIds = [];
            foreach ($newTags as $tagName) {
                $createdIds[] = Tag::create([
                    'name' => $tagName,
                    'created_by' => getLoggedInUserId(),
                ])->id;
            }
            if (!empty($createdIds)) {
                $task->tags()->attach($createdIds);
            }
        }
    ...
    public function updateTaskStatus($id, $input)
    {
        ...
        // Get the status record to determine if it's a completed status
        $statusRecord = \App\Models\Status::where('status', $newStatus)->first();
        $isCompletedStatus = $statusRecord && strcasecmp($statusRecord->name, 'completed') == 0;
        
        // Set completed_on based on status
        $completedOn = $isCompletedStatus ? Carbon::now() : null;
