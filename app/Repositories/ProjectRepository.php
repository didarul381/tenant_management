...
class ProjectRepository extends BaseRepository
{
     ...
     * @param  array  $users
     */
    public function sendMailToUsersOld($users)
    {
        foreach ($users as $user) {
        ...
        foreach ($users as $user) {
            ...
            if (! empty($user->email)) {
                try {
                    ...
                    Log::warning('Failed to send project assignment email to '.$user->email.': '.$e->getMessage());
                    // Do not rethrow — email failures should not block project create/update
                }
            }
        }
    }
