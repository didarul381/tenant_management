...
class UserController extends AppBaseController
{
    ...
    public function index(Request $request, RoleRepository $roleRepository, ProjectRepository $projectRepository)
    {
        ...
        return view('users.index')->with(['projects' => $projects, 'roles' => $roles, 'status' => $status, 'departments' => $departments]);
    }
    ...
    {
        $user = User::with('media', 'projects', 'client', 'userActiveTask', 'department')->findOrFail($user->id);
        ...
        return view('users.show', compact('user', 'projects', 'roles','departments'));
    }
    ...
    public function edit(User $user)
    {
        ...
        $userObj['department_id'] = $user->department_id;

        // Calculate project statistics
        $totalProjects = $user->projects()->count();
        $projectsWithIncompleteTasks = 0;

        foreach ($user->projects as $project) {
            $incompleteTasksCount = $project->tasks()
                ->where('status', '!=', 1) // status 1 means complete
                ->count();
            
            if ($incompleteTasksCount > 0) {
                $projectsWithIncompleteTasks++;
            }
        }

        $userObj['project_stats'] = [
