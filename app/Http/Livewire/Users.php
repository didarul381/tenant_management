...
class Users extends SearchableComponent
{
    ...
    {
        $this->departments = Department::orderBy('name', 'asc')->pluck('name', 'id')->toArray();
    }
    ...
    public function searchUser($role)
    {
        $this->setQuery($this->getQuery()->with('roles', 'media', 'client')->where('owner_id', null)->where('owner_type', null)->orderByDesc('is_active')->withCount('projects',
            'taskAssignee', 'userActiveTask')->orderByDesc('created_at'));
        ...
        // Filter by department if selected (including non-departmental)
        if ($this->departmentId === 'none') {
            $this->getQuery()->whereNull('department_id');
        } else {
            $this->getQuery()->when(! empty($this->departmentId), function (Builder $q) {
