...
window.actionTemplate2 = Handlebars.compile(
    '<button type="button" id="createFollowUpBtn" class="btn createFollowUpBtn" style="color: #47c363;" data-id="{{id}}" title="New Follow Up">' +
        '<i class="fa fa-plus"></i>' +
    '</button>' +
    '<a title="{{viewText}}" class="mt-1 mr-2 card-view-icon" href="{{Url}}/{{id}}">' +
...
let notifications = Handlebars.compile(
    '<a href="#" data-id="{{id}}" class="dropdown-item dropdown-item-unread notification-link-icon"\n' +
    '                   data-link="{{link}}">\n' +
    ...
    '                            <span class="readNotification" data-id="{{id}}" title="Dismiss"\n' +
    '                                  style="display:inline-flex;align-items:center;margin-left:6px;color:#6c757d;cursor:pointer;">\n' +
        ...
        success: function (result) {
            if (result.success) {
                ...
                if (result.data.length > 0) {
                    ...
                    $('#notificationCount').removeClass('d-none').text(result.data.length);
                    ...
                    // hide notification count badge when no unread
...
// handle click on notification area (not the cross icon) - navigate to project
$(document).on('click', '.notification-link-icon', function (e) {
    e.preventDefault();
    e.stopPropagation();
    ...
    // mark as read first, update UI counts, then navigate
    $.ajax({
        type: 'POST',
        ...
        success: function () {
            ...
            $item.remove();
            let notificationCounter = document.getElementsByClassName('readNotification').length;
            if (notificationCounter == 0) {
                $('#allRead').addClass('d-none');
                $('.empty-notification').removeClass('d-none');
                $('.nav-link.notification-toggle').removeClass('beep');
            }
            // update badge count after removing one
            if (notificationCounter > 0) {
                $('#notificationCount').removeClass('d-none').text(notificationCounter);
            } else {
                $('#notificationCount').addClass('d-none').text('0');
            }
            
            // navigate to project after marking as read
            if (link && typeof link === 'string' && link.length > 0) {
                window.location.href = link;
            }
        },
...
// handle click on cross icon - just dismiss notification without navigation
$(document).on('click', '.readNotification', function (e) {
    e.preventDefault();
    e.stopPropagation();
    let notificationId = $(this).data('id');
    let notification = $(this).closest('.notification-link-icon');
    notification.remove();
    $.ajax({
        type: 'POST',
        url: '/notification/' + notificationId + '/read',
