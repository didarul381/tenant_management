...
let drake = dragula({
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            try {
                if (window.Livewire) {
                    window.Livewire.find(window.livewire.components.firstComponentId()).call('$refresh');
                } else if (window.livewire) {
                    window.livewire.emit('refresh');
                }
            } catch (e) {
                console.log('Livewire refresh failed, reloading page...');
        ...
        error: function(xhr) {
            // Show error message and revert the drag
            var message = xhr.responseJSON ? xhr.responseJSON.message : 'Error updating task status';
            
            // Show notification (assuming you have a notification function)
            ...
            // Revert the task back to its original position
            setTimeout(function() {
                location.reload();
            }, 500);
        },
...
$(document).ready(() => {
    ...
    $(document).on('change', '#projectDropdown', function () {
        $('#usersDropdown').empty();
        let id = $(this).val();
        let url = id ? route('users-by-project', id) : route('users-by-project');
        console.log(id);
       // If "Select One" selected
        // if (id === '' || id === null) {
        //     window.livewire.emit('loadByProject', '');
        //     return;
        // }
        $.ajax({
            url: url,
