...
let taskAssignees = [];
// Accumulate selected files across multiple selections before submit
let createTaskFiles = [];

// When user picks files multiple times, keep all of them instead of replacing
$(document).on('change', '#Add_attachment', function (e) {
    const files = e.target.files || []
    for (let i = 0; i < files.length; i++) {
        createTaskFiles.push(files[i])
    }
...
$('#taskEstimateTimeHours').hide()

$(document).on('click', '#taskMinutes', function () {
    ...
    $('#taskEstimateTimeMinutes').show()
})
$(document).on('click', '#taskHours', function () {
    $('#taskDays').removeClass('btn btn-primary text-white')
    $('#taskMinutes').removeClass('btn btn-primary text-white')
    $(this).addClass('btn btn-primary text-white')
    $('#taskTypes').val(0)
    $('#taskEstimateTimeDays').hide().val('')
    $('#taskEstimateTimeMinutes').hide().val('')
...
$(document).on('submit', '#addTaskNewForm', function (event) {
    ...
    // Append all accumulated files (supports multiple selections before submit)
    if (createTaskFiles.length > 0) {
        for (let i = 0; i < createTaskFiles.length; i++) {
            formData.append('file[]', createTaskFiles[i])
        }
    } else {
        // Fallback to current input if no accumulated files
...
$('#addTaskModal').on('hidden.bs.modal', function () {
    ...
    $('#taskEstimateTimeHours').data('DateTimePicker').date('00:00')
    $('#taskMinutes').addClass('btn btn-primary text-white')
    $('#taskTypes').val(2)
    $('#taskEstimateTimeDays').hide().val('')
    $('#taskEstimateTimeHours').hide().val('')
    $('#taskEstimateTimeMinutes').show().val('')
    $('#previewImage').empty()
    createTaskFiles = []
    $('#Add_attachment').val('')
