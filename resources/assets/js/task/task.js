    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                // Load project assignees and preselect only once after options are populated
                // Ensure unique IDs to avoid duplicates
                var uniqueUserIds = Array.from(new Set(userIds.map(function(v){ return String(v); })));
                loadProjectAssignees(task.project.id, 'editAssignee', uniqueUserIds);

                $('#editPriority').val(task.priority).trigger('change')

                // Show modal after a short delay to ensure select2 has rendered options
                setTimeout(function () {
...
$('#AddModal').on('hidden.bs.modal', function () {
    ...
    $('#Minutes').addClass('btn btn-primary text-white');
...
$(function () {
    ...
    updateTaskStatus = (id) => {
        $.ajax({
            url: route('task.update-status3',id),
            type: 'POST',
            cache: false,
            success: function (result) {
                if (result.success) {
             ...
             error: function (xhr) {
            // Static error message for HTTP errors
            swal({
                'title': 'Error',
                'text': 'Need to Assaign Time and User To Complete Task',
                'type': 'error',
                confirmButtonColor: '#6777EF',
            });
...
function loadProjectAssignees (projectId, selector, preselectedIds) {
    editTaskAssignees = [];
    $('#' + selector).empty();
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            // If preselectedIds provided, select them once (deduped)
            if (Array.isArray(preselectedIds) && preselectedIds.length > 0) {
                var unique = Array.from(new Set(preselectedIds.map(function(v){ return String(v); })));
                $('#' + selector).val(unique).trigger('change');
            } else {
...
$(document).ready(function () {
    ...
    $('#estimateTimeHours').hide();

     $(document).on('click', '#Minutes', function () {
        $('#Minutes').removeClass('btn btn-primary text-white');
        $(this).addClass('btn btn-primary text-white');
        $('#types').val(2);
        $('#estimateTimeDays').hide().val('');
        $('#estimateTimeHours').hide().val('');
        $('#estimateTimeMinutes').show();
    });
