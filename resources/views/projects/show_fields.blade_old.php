            ...
            @if($shouldShowSummary)
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <div class="col-6 commision-summary">
                        ...
                        <p class="mb-0">Job On Boarded: {{ $onboardUserName }}. 4% Commission Earned ({{ number_format($onboardCommission) }} BDT)</p>
                        <div>
                            <p class="mb-1">Team Commission (4%)</p>
                            @forelse($deptUsers as $user)
                                @php
                                    $isOnboarder = $onboardUser && $user->id === $onboardUser->id;
                                    $amount = $teamCommissionPerUser + ($isOnboarder ? $onboardCommission : 0);
                                @endphp
                                <p class="mb-0">
                                    {{ $user->name }} : {{ number_format($amount) }} BDT
                                    @if($isOnboarder && $onboardCommission > 0)
                                        <span>(Includes {{ number_format($onboardCommission) }} BDT as Onboarder)</span>
                                    @endif
                                </p>
                            @empty
                                <p class="mb-0">No users found in onboarder's department.</p>
                            @endforelse
                        </div>
                    </div>
                </div>
