<div class="row">
    ...
    @empty
        <div class="mt-0 mb-5 col-12 d-flex justify-content-center mb-5 rounded">
            <div class="p-2">
                @if(empty($search))
                    <p class="text-dark">{{ __('messages.job-type.no_type_available') }}</p>
                @else
                    <p class="text-dark">{{ __('messages.job-type.no_type_found') }}</p>
                @endif
            </div>
        </div>
    @endforelse
        ...
        <div class="row paginatorRow">
            <div class="col-lg-2 col-md-6 col-sm-12 pt-2">
                @if($totalType != 0)
                    ...
                    <span class="font-weight-bold ml-1 mr-1">{{ $type->firstItem() }}</span> - 
                    <span class="font-weight-bold ml-1 mr-1">{{ $type->lastItem() }}</span> {{ __('messages.common.of') }} 
                    <span class="font-weight-bold ml-1">{{ $type->total() }}</span>
                </span>
                @endif
            </div>
